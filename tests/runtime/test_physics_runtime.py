...
def test_broad_phase_discards_distant_pairs_without_changing_contacts() -> None:
    scene = _empty_editor_scene()
    world = PhysicsWorld()
    for index in range(50):
        obj = _add_object(scene, f"Object{index}", float(index * 100), 0.0)
        world.register_collider(obj.add_component(BoxCollider(width=16, height=16)))
    assert world.detect_collisions() == []
    assert world.broad_phase_candidates == 0
