...
class Camera3D(Component):
    ...
    def __init__(self, fov: float = 60.0, near: float = 0.1, far: float = 100.0,
                 ...
                 viewport_width: Optional[float] = None, viewport_height: Optional[float] = None) -> None:
        super().__init__()
        self.fov = fov
        self.near = near
        self.far = far
    ...
    def update(self, dt: float) -> None:
        ...
        if self.game_object:
            ...
            # Recalculate projection based on display/viewport size
...
class MeshRenderer3D(Component):
    ...
    Supports wireframe, filled polygons, and basic flat shading.
    """
    def __init__(self, mesh: Mesh, color: Tuple[int, int, int] = (200, 200, 200), 
        ...
        # Simple global directional light vector (pointing down, right, forward)
    ...
    def draw(self, screen: pygame.Surface) -> None:
        ...
        m_matrix = translation_matrix(pos[0], pos[1], pos[2]) @ \
        ...
        # 2. Project vertices to NDC and get depths
        ...
        # Convert all NDC to Screen space pixels in one go
        screen_coords = np.zeros((ndc_coords.shape[0], 2), dtype=np.int32)
        screen_coords[:, 0] = v_x + (ndc_coords[:, 0] + 1.0) * w_half
        screen_coords[:, 1] = v_y + (-ndc_coords[:, 1] + 1.0) * h_half  # Flip Y
        self.last_screen_coords = screen_coords

        # 3. Model Rotation Matrix (for transforming face normals to world space)
        # Scale doesn't affect normals direction if uniform, but rotation does
        rot_m = rotation_matrix(rot[0], rot[1], rot[2])[:3, :3]
        
        # 4. Filter and sort faces (Painter's Algorithm)
        faces_to_draw = []
        near_plane = Camera3D.main.near
        
        for i, face in enumerate(self.mesh.faces):
            # Check if any vertex is behind the camera (near plane clipping)
            face_depths = depths[face]
            if np.any(face_depths < near_plane):
                continue  # Skip clipping face
                
            # Backface culling:
            # Transformed face normal in world space
            face_normal = self.mesh.face_normals[i]
            world_normal = rot_m @ face_normal
            
            # Vector from camera to face center
            # We use first vertex of the face transformed to world space for simplicity
            ...
            if cam_to_face_norm > 0:
                cam_to_face = cam_to_face / cam_to_face_norm
            
            # If normal points away from camera direction, cull it
            ...
            # Normalize to [0.1, 1.0] range for ambient light buffer
