        ...
        try:
            with open(file, "r", encoding="utf-8") as f:
                ...
                if isinstance(data, dict):
                    # In a real scenario we could catch duplicates here
...
def run_validation(locales_dir: Path, base_locale: str = "en-US") -> bool:
    ...
    for loc_dir in locales_dir.iterdir():
        if loc_dir.is_dir() and loc_dir.name != base_locale:
            ...
            # Check empty values and placeholders
            ...
            if empty_values > 0:
                print(f"  Found {empty_values} empty values.")
            if orphan:
                print(f"  Warning: Found {len(orphan)} orphan keys.")
