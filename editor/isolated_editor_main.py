...
<<<<<<< HEAD
from PySide6.QtWidgets import QFileDialog, QInputDialog, QMenu, QToolBar, QHBoxLayout, QFormLayout, QCheckBox, QLabel, QComboBox, QPushButton
=======
...
>>>>>>> 64dce925b8d9327696243e0af5ea53432c96821f
...
class IsolatedEditorWindow(InterfaceSmokeTest):
    ...
    def _preview_asset(self, item: QTreeWidgetItem) -> None:
        ...
        if not path.exists() or path.is_dir():
            ...
            self.preview_label.setText(f"Diretório:\n{path.name}")
            return
        ...
        if ext in {".png", ".jpg", ".jpeg", ".bmp", ".webp"}:
            ...
            if not pixmap.isNull():
                self.preview_label.setPixmap(pixmap.scaled(180, 130, Qt.KeepAspectRatio, Qt.SmoothTransformation))
                meta_html = (
                    f"<b>{path.name}</b><br><br>"
                    f"Tipo: Textura 2D<br>"
                    f"Tamanho: {pixmap.width()} x {pixmap.height()}<br>"
                    f"Formato: {ext[1:].upper()}<br>"
                    f"Tamanho: {size_str}<br>"
                    f"Importado: {date_str}"
                )
                # Tenta atualizar o layout de metadados se existir ou apenas adicionar info
                self.preview_label.setToolTip(meta_html)
                # Vamos injetar o html formatado na label de detalhes
                self._update_preview_details(meta_html)
                return
        ...
        # Para scripts
            ...
            meta_html = (
                f"<b>{path.name}</b><br><br>"
                f"Tipo: Script Python<br>"
                f"Tamanho: {size_str}<br>"
                f"Linguagem: Python 3<br>"
                f"Importado: {date_str}"
            )
            self.preview_label.clear()
            self.preview_label.setText("🐍 Script Component")
            self._update_preview_details(meta_html)
            return
        ...
        # Para Cenas
            ...
            meta_html = (
                f"<b>{path.name}</b><br><br>"
                f"Tipo: Cena Zennity<br>"
                f"Tamanho: {size_str}<br>"
                f"Formato: JSON Scene<br>"
                f"Importado: {date_str}"
            )
            self.preview_label.clear()
            self.preview_label.setText("🎬 Scene Asset")
            self._update_preview_details(meta_html)
            return
        ...
        meta_html = (
            f"<b>{path.name}</b><br><br>"
            f"Tipo: Arquivo Genérico<br>"
            f"Tamanho: {size_str}<br>"
            f"Extensão: {ext.upper()}<br>"
            f"Importado: {date_str}"
        )
        self.preview_label.clear()
    ...
    def _update_preview_details(self, html: str) -> None:
        if hasattr(self, "preview_details_label"):
            self.preview_details_label.setText(html)
