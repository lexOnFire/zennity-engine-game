...
class InspectorComponentController:
    ...
    def choose_sprite_texture(self) -> None:
        ...
        _, obj = selected
        h = self.host
        filename, _ = QFileDialog.getOpenFileName(h, "Selecionar textura", str(Path.cwd() / "Assets"), "Imagens (*.png *.jpg *.jpeg *.bmp *.webp)")
        if not filename:
            return
        path = Path(filename)
        try:
    ...
    def choose_ui_image(self) -> None:
        if self._selected() is None:
            return
        h = self.host
        filename, _ = QFileDialog.getOpenFileName(h, "Selecionar imagem da UI", str(Path.cwd() / "Assets"), "Imagens (*.png *.jpg *.jpeg *.bmp *.webp)")
        if not filename:
            return
        path = Path(filename)
        try:
    ...
    def send_ui(self) -> None:
        ...
        ui.update({
            "visible": h.show_ui_chk.isChecked(), "text": h.ui_text_field.text(),
            "path": h.ui_image_path_field.text(), "interactable": h.ui_interactable_field.isChecked(),
    ...
    def ensure_canvas(self) -> None:
        ...
        canvas = {
            ...
            "mesh_type": "UI", "renderer_enabled": False, "ui": normalize_ui({"type": "canvas"}),
        }
